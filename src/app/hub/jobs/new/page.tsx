...
export default function NewJobPage() {
  ...
  return (
    <div className="mx-auto max-w-lg space-y-6">
      <Link href="/hub/jobs" className="text-sm text-cyan-400 hover:underline">
        ← Jobs
      </Link>
      ...
      <form className="space-y-4" onSubmit={form.handleSubmit(onSubmit)}>
        {(["companyName", "title", "location"] as const).map((f) => (
          <label key={f} className="block text-sm text-slate-400">
            {f}
            <input
        ...
        <button
          type="submit"
          className="w-full rounded-lg bg-cyan-600 py-2.5 text-sm font-semibold text-white"
        >
          Publish
        </button>
      </form>
