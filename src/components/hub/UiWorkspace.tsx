  ...
  return (
    <div className="grid gap-6 lg:grid-cols-2">
      <div className="space-y-4">
        ...
        <p className="text-sm text-slate-400 whitespace-pre-wrap">{props.description}</p>
        <div className="flex flex-wrap gap-2">
          <label className="text-sm text-slate-400">
            Framework template
            <select
              className="ml-2 rounded-lg border border-slate-600 bg-slate-950 px-2 py-1"
              value={fw}
          ...
          <button
            type="button"
            onClick={submit}
            className="rounded-lg bg-cyan-600 px-4 py-1 text-sm font-semibold text-white"
          >
            Submit
          </button>
        </div>
      ...
      <div>
        <h2 className="mb-2 text-sm font-medium text-slate-400">Live preview</h2>
        <iframe
          title="UI preview"
          sandbox="allow-scripts allow-same-origin"
          className="h-[480px] w-full rounded-xl border border-slate-700 bg-white"
          srcDoc={html}
        />
      </div>
    </div>
