...
export default function TrustBar() {
  return (
    <section
      className="border-y border-[var(--color-border)]/40 bg-[var(--surface)]"
      ...
      <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6 lg:px-8">
        <div className="flex flex-col items-center gap-2 sm:flex-row sm:justify-center sm:gap-4">
          ...
          <div className="flex items-center gap-1.5 text-[var(--color-muted)]">
            <svg className="h-4 w-4 text-[var(--color-primary)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden>
        ...
        <div className="mt-8 grid grid-cols-2 gap-6 sm:grid-cols-4 sm:gap-8">
          {stats.map((stat, i) => (
            <div key={stat.label} className="text-center">
              ...
              <p className="mt-1 text-sm font-medium text-[var(--color-muted)]">{stat.label}</p>
            </div>
          ))}
        </div>
        <div className="mt-10 border-t border-[var(--color-border)]/40 pt-8">
          <p className="text-center text-xs font-medium uppercase tracking-widest text-[var(--color-muted)]">
            Trusted globally
          </p>
          <div className="mt-4 flex flex-wrap items-center justify-center gap-x-6 gap-y-2 text-sm font-medium text-white/90 sm:gap-x-10">
            {regions.map((region) => (
              <span key={region} className="transition-colors hover:text-white">{region}</span>
            ))}
          </div>
        </div>
      </div>
